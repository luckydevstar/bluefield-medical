...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin
    .from('locations')
    .insert({ name, address, postcode, lat, lng })
    .select('*')
    .single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PATCH(req: NextRequest) {
  ...
  const { id, name, address, postcode, lat, lng } = await req.json();
  ...
  if (lat !== undefined) {
    const v = lat === null ? null : Number(lat);
    patch.lat = Number.isFinite(v) ? v : null;
  }
  if (lng !== undefined) {
    const v = lng === null ? null : Number(lng);
    patch.lng = Number.isFinite(v) ? v : null;
  }
...
export async function DELETE(req: NextRequest) {
  ...
  // ... (your existing cascade/guard logic unchanged)
  // 1) service day ids
  ...
  const { error: delLoc } = await supabaseAdmin.from('locations').delete().eq('id', id);
