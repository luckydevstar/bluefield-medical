...
interface HearingLossFAQAdminFormProps {
  isOpen: boolean;
  onClose: () => void;
  data: FAQData;
...
export function HearingLossFAQAdminForm({ isOpen, onClose, data, onSave }: HearingLossFAQAdminFormProps) {
    ...
    <Drawer open={isOpen} onOpenChange={(open) => !open && onClose()} direction="bottom">
      <DrawerContent className="mx-auto h-screen max-w-3xl">
        <DrawerHeader>
          ...
          <DrawerDescription>Manage frequently asked questions</DrawerDescription>
        </DrawerHeader>
